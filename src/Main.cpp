...
static sf::Image Render(const uint32_t initialWidth, const uint32_t initialHeight)
{
	...
	std::jthread renderThread([&]
	{
		...
		while (isRunning.load(std::memory_order_relaxed))
		{
			...
			{
				timeTaken = cpuRenderer.Render(sharedDimensions, image).count();
	...
	while (true)
	{
		...
		window.setTitle(fmt::format("Ray Tracing! {}x{} | {} Time: {:.3f}ms", dims.x, dims.y, executionMode == ExecutionMode::CPU ? "CPU" : "GPU", timeTaken));
	}
