...
template<>
template<>
__host__ std::chrono::duration<float, std::milli> Renderer<ExecutionMode::CPU>::Render<sf::Image>(const sf::Vector2u& size, sf::Image& surface)
{
				...
				pool.Enqueue([&, tileX, tileY]()
			{
				...
				for (uint32_t y = tileY; y < yEnd; ++y)
				{
					for (uint32_t x = tileX; x < xEnd; ++x)
					{
						...
						surface.setPixel(
							{x, y},
							{static_cast<uint8_t>(pixelColor.x * 255.f),
							 static_cast<uint8_t>(pixelColor.y * 255.f),
							 static_cast<uint8_t>(pixelColor.z * 255.f),
							 255});
					}
				}
			}));
